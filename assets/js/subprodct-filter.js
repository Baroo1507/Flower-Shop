...
// Initialize data with the full product list
let data = product; 

// Filter by category
let filteredCategoryData = product.filter(item => item.category === selectedCategory);
if (filteredCategoryData.length > 0) {
    data = filteredCategoryData; // Overwrite data with filtered products by category
}

// Check if subcategory is specified and filter if necessary
if (selectedSubcategory) {
    const filteredSubcategoryData = data.filter(item => item.subcategory === selectedSubcategory);
    if (filteredSubcategoryData.length > 0) {
        data = filteredSubcategoryData; // Overwrite data with filtered products by subcategory
    }
}
...
function renderProducts(data) {
    // Clear all existing products in the container
    productContainer.innerHTML = "";

    // Iterate through data array and create products
